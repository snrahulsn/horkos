...
export async function ingestGitHubCheckRun(
  body: string,
  signature: string | undefined,
  event: string | undefined,
) {
  if (!validSignature(body, signature)) throw new Error('invalid GitHub webhook signature');
  if (event !== 'check_run') return { accepted: true, matched: 0 };
  ...
  const { rows } = await pool.query(
    `SELECT m.id AS milestone_id, m.oath_id
     ...
     WHERE m.criteria_type = 'github_check'
       ...
       AND m.status = 'OPEN' AND o.status IN ('OPEN','CLAIMED')`,
    [repo, run.head_sha, run.name],
  );
  ...
  for (const row of rows) {
    ...
    if (run.conclusion === 'success') await recordVerifiedAssertion(secret, input);
    else await recordRejectedAssertion(secret, input);
  }

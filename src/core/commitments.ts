...
import { validateCommitment, type CommitmentInput } from './guardrails.js';
 ...
 */
export async function createCommitment(agentId: string, raw: unknown) {
  const now = new Date();
  const result = validateCommitment(raw, now);
  if (!result.ok || !result.data) throw new GuardrailError(result.errors);
  const c = result.data;

  return tx(async (client) => {
    // Locked identity: postmortem outstanding
    const agent = await client.query(
      `SELECT locked, locked_oath_id FROM agents WHERE id = $1`,
      [agentId],
    );
    if (!agent.rows.length) throw new GuardrailError(['unknown agent']);
    ...
    for (let i = 0; i < c.milestones.length; i++) {
      ...
      await client.query(
        `INSERT INTO milestones (oath_id, position, title, criteria_type, criteria_detail, deadline, budget_slice_usd)
         VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [oathId, i + 1, m.title, m.criteria.type, JSON.stringify(m.criteria), m.deadline, m.budget_slice_usd],
      );
    }
...
export async function activateOath(rawToken: string) {
  const now = new Date();
  return tx(async (client) => {
    const { rows } = await client.query(
      `SELECT id, ref, status, draft_expires_at FROM oaths WHERE counterparty_token = $1 FOR UPDATE`,
      [sha256Hex(rawToken)],
    );
    if (!rows.length) throw new GuardrailError(['invalid or used activation token']);
    const oath = rows[0];
    ...
    await client.query(
      `UPDATE oaths SET status = 'OPEN', activated_at = $2, void_until = $3, counterparty_token = $4
       WHERE id = $1`,
      [oath.id, now, new Date(now.getTime() + VOID_WINDOW_MS), sha256Hex(nextToken)],
    );
...
export async function voidOath(oathId: string, counterpartyToken: string) {
  const now = new Date();
  return tx(async (client) => {
    const { rows } = await client.query(
      `SELECT id, ref, status, void_until, counterparty_token FROM oaths WHERE id = $1 FOR UPDATE`,
      [oathId],
    );
    if (!rows.length) throw new GuardrailError(['unknown oath']);
    const oath = rows[0];
    if (oath.status !== 'OPEN') throw new GuardrailError([`oath is ${oath.status}`]);
    ...
    const claims = await client.query(
      `SELECT 1 FROM claims c JOIN milestones m ON c.milestone_id = m.id WHERE m.oath_id = $1 LIMIT 1`,
      [oathId],
    );
    if (claims.rows.length) throw new GuardrailError(['cannot void after a claim is filed']);

    await client.query(
      `UPDATE oaths SET status = 'VOIDED', resolved_at = $2 WHERE id = $1`,
      [oathId, now],
    );
...
export async function proposeAmendment(
  ...
  field: 'deadline' | 'budget' | 'criteria',
  oldValue: unknown,
  newValue: unknown,
) {
  return tx(async (client) => {
    const { rows } = await client.query(
      `SELECT ref, status FROM oaths WHERE id = $1`,
      [oathId],
    );
    if (!rows.length) throw new GuardrailError(['unknown oath']);
    ...
    const res = await client.query(
      `INSERT INTO amendments (oath_id, milestone_id, field, old_value, new_value)
       VALUES ($1,$2,$3,$4,$5) RETURNING id`,
      [oathId, milestoneId, field, JSON.stringify(oldValue), JSON.stringify(newValue)],
    );
...
export async function approveAmendment(amendmentId: string, counterpartyToken: string) {
  const now = new Date();
  return tx(async (client) => {
    const { rows } = await client.query(
      `SELECT a.*, o.ref, o.counterparty_token AS oath_token
       FROM amendments a JOIN oaths o ON a.oath_id = o.id WHERE a.id = $1 FOR UPDATE OF a`,
      [amendmentId],
    );
    if (!rows.length) throw new GuardrailError(['unknown amendment']);
    const am = rows[0];
    if (am.approved_at) throw new GuardrailError(['already approved']);
    ...
    if (am.field === 'deadline') {
      ...
      await client.query(
        `UPDATE milestones SET criteria_type = $2, criteria_detail = $3 WHERE id = $1`,
        [am.milestone_id, (newVal as any).type, JSON.stringify(newVal)],
      );
    }

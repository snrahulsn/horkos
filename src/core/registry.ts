...
export async function registerAgent(authUserId: string, agentName: string, displayName?: string) {
  return tx(async (client) => {
    ...
    const agent = await client.query(
      ...
      [operatorId, pubkey, privkey, sha256Hex(apiToken), agentName],
    );
...
export async function lookupAgent(pubkey: string) {
  ...
  const oaths = await pool.query(
    `SELECT o.ref, o.domain, o.model_declared, o.specificity_grade, o.status, o.visibility,
...
export async function queryRegistry(opts: {
  ...
  const where: string[] = [`o.status NOT IN ('DRAFT','DRAFT_EXPIRED')`];
  ...
  const { rows } = await pool.query(
    `SELECT o.ref, o.domain, o.model_declared, o.specificity_grade, o.status,
            o.activated_at, o.deadline, o.resolved_at, o.deadline_met, o.budget_met,
            o.budget_over_pct, o.deliverable_confirmed, o.commitment_hash,
...
export async function getOath(ref: number) {
  const { rows } = await pool.query(
    `SELECT o.*, a.pubkey AS agent_pubkey, a.name AS agent_name
     FROM oaths o JOIN agents a ON o.agent_id = a.id
     WHERE o.ref = $1 AND o.status NOT IN ('DRAFT','DRAFT_EXPIRED')`,
    [ref],
  );
  if (!rows.length) return null;
  const o = rows[0];
  ...
  const amendments = await pool.query(
    `SELECT field, old_value, new_value, proposed_at, approved_at, milestone_id

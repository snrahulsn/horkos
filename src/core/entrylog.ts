...
export async function logEvent(
  ...
  await client.query(
    `INSERT INTO entry_log (event_type, payload, prev_hash, this_hash) VALUES ($1, $2, $3, $4)`,
    [eventType, canonical, prevHash, thisHash],
  );
}

/** Verify the whole chain. Returns first broken seq, or null if intact. */
export async function verifyChain(client: pg.PoolClient): Promise<bigint | null> {
  const { rows } = await client.query(
    `SELECT seq, event_type, payload, prev_hash, this_hash FROM entry_log ORDER BY seq`,
  );
  let prev = GENESIS;
  for (const row of rows) {
    if (row.prev_hash !== prev) return BigInt(row.seq);
    const expected = sha256Hex(row.prev_hash + row.event_type + canonicalJson(row.payload));
    if (row.this_hash !== expected) return BigInt(row.seq);
    prev = row.this_hash;
  }
  return null;
}

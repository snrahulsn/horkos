 ...
 */
export async function buildRollups(granularity: 'hour' | 'day') {
...
export async function queryStats(raw: unknown) {
  ...
  const { rows } = await pool.query(
    `SELECT bucket_start, model, domain, oaths_opened, oaths_resolved, milestones_resolved,
            kept, broken, broken_unconfirmed, disputed, voided, withdrawn,
            mean_budget_over_pct, mean_attempts, mean_duration_s, rca_filed, incidents_filed
     FROM rollups WHERE ${where}
     ORDER BY bucket_start DESC LIMIT $${params.length}`,
    params,
  );
  return rows;
}

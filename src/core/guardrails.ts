...
const HEDGE_PHRASES =
  /\b(try to|attempt to|hopefully|maybe|might|should be able|best effort|aim to|do my best|probably|i think|around \d+% (sure|confident)|\d+% (sure|confident|certain))\b/i;

function rejectConfidenceKeys(obj: unknown, path = ''): string | null {
  if (obj === null || typeof obj !== 'object') return null;
  for (const [key, val] of Object.entries(obj)) {
    if (CONFIDENCE_KEYS.test(key)) return `${path}${key}`;
...
export interface ValidationResult {
  ok: boolean;
  errors: string[];
  specificityGrade?: 'A' | 'B' | 'C';
  data?: CommitmentInput; // parsed, with defaults applied
}

export function validateCommitment(raw: unknown, now: Date): ValidationResult {
  const errors: string[] = [];
  ...
  for (const [where, text] of [
    ['goal', c.goal] as const,
    ...c.milestones.map((m, i) => [`milestones[${i}].title`, m.title] as const),
  ]) {
    const hedge = text.match(HEDGE_PHRASES);
    if (hedge) {
      errors.push(
        `${where}: hedge language "${hedge[0]}" rejected. Swear a measurable outcome or do not swear.`,
      );
    }
  }
  ...
  if (parentDeadline <= now) errors.push('deadline: must be in the future');
  let prevDeadline = now;
  c.milestones.forEach((m, i) => {
    const d = new Date(m.deadline);
    if (d <= now) errors.push(`milestones[${i}].deadline: must be in the future`);
    if (d > parentDeadline)
  ...
  if (sliceSum > c.budget_cap_usd + 1e-9) {
    errors.push(
      `milestones: budget slices sum to $${sliceSum.toFixed(2)}, exceeding cap $${c.budget_cap_usd.toFixed(2)}`,
    );
  }
 ...
 */
export function gradeSpecificity(c: CommitmentInput): 'A' | 'B' | 'C' {
  let score = 0;
  ...
  if (/\d/.test(c.goal) || /[`"'][^`"']+[`"']/.test(c.goal)) score += 1;

  // Decomposition: more than one milestone shows a thought-through path
  if (c.milestones.length >= 2) score += 1;

  // Short vague goal is weak
  if (c.goal.length < 40) score -= 1;
...
export function evidenceMatchesCriteria(evidence: ClaimInput['evidence'], criteria: Criteria): string | null {
  ...
  if (criteria.type === 'metric_threshold' && evidence.type === 'metric_threshold') {
    ...
    const pass =
      ...
      v === threshold;
    if (!pass) return `measured value ${v} fails ${operator} ${threshold}`;
  }

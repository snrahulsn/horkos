...
export async function publishMerkleRoot() {
  ...
  if (!seed) return; // unsigned deployments skip publication
  ...
  const { rows } = await pool.query(
    `SELECT seq, this_hash FROM entry_log WHERE seq >= $1 ORDER BY seq`,
    [fromSeq],
  );
  ...
  await pool.query(
    `INSERT INTO merkle_roots (from_seq, to_seq, root, signature) VALUES ($1,$2,$3,$4)`,
    [fromSeq, rows[rows.length - 1].seq, root, signature],
  );
...
export async function listMerkleRoots(limit = 100) {
  const { rows } = await pool.query(
    `SELECT from_seq, to_seq, root, signature, signed_at FROM merkle_roots ORDER BY signed_at DESC LIMIT $1`,
    [limit],
  );
  return rows;
}

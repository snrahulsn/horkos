...
async function tick() {
  ...
  await pool.query(
    `UPDATE oaths SET status = 'DRAFT_EXPIRED', resolved_at = now() WHERE status = 'DRAFT' AND draft_expires_at < now()`,
  );

  // 2. Milestone deadlines: OPEN + deadline passed -> BROKEN
  const brokenMs = await pool.query(
    `SELECT m.id, m.position, o.id AS oath_id, o.ref FROM milestones m
     JOIN oaths o ON m.oath_id = o.id
     WHERE m.status = 'OPEN' AND m.deadline < $1 AND o.status IN ('OPEN','CLAIMED')`,
    [now],
  );
  for (const m of brokenMs.rows) {
    await tx(async (client) => {
      await client.query(
        `UPDATE milestones SET status = 'BROKEN', resolved_at = $2, deadline_met = false WHERE id = $1 AND status = 'OPEN'`,
        [m.id, now],
      );
  ...
  const silent = await pool.query(
    `SELECT c.id AS claim_id, m.id AS milestone_id, m.position, m.deadline AS m_deadline,
     ...
     WHERE c.counterparty_response IS NULL AND c.response_due < $1 AND m.status = 'CLAIMED'`,
    [now],
  );
  for (const s of silent.rows) {
    await tx(async (client) => {
      ...
      await client.query(
        `UPDATE milestones SET status = 'BROKEN_UNCONFIRMED', resolved_at = $2, deadline_met = $3, budget_met = $4
         WHERE id = $1 AND status = 'CLAIMED'`,
        [s.milestone_id, now, deadlineMet, budgetMet],
      );
  ...
  const expiredParents = await pool.query(
    `SELECT id, ref FROM oaths WHERE status IN ('OPEN','CLAIMED') AND deadline < $1
     AND NOT EXISTS (
    ...
    await tx(async (client) => {
      await client.query(
        `UPDATE milestones SET status = 'BROKEN', resolved_at = $2, deadline_met = false
         WHERE oath_id = $1 AND status = 'OPEN'`,
        [p.id, now],
      );

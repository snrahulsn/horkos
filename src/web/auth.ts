...
export function makeSession(user: SupabaseUser): string {
  const body = Buffer.from(
    JSON.stringify({ id: user.id, email: user.email, exp: Math.floor(Date.now() / 1000) + MAX_AGE_S }),
  ).toString('base64url');
  return `${body}.${sign(body)}`;
}

export function readSession(cookieValue: string | undefined): SupabaseUser | null {
  ...
  try {
    ...
    if (!p.id || (p.exp && p.exp * 1000 < Date.now())) return null;

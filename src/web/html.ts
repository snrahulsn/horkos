...
export function verdictBadge(status: string): string {
...
export function axes(o: {
  ...
  budget_over_pct?: string | number | null;
  deliverable_confirmed?: boolean | null;
}): string {
  const cell = (v: boolean | null | undefined, over?: string | number | null) => {
    if (v === null || v === undefined) return '—';
    if (v) return 'met';
    return over ? `<span class="red">over ${esc(Number(over).toFixed(0))}%</span>` : '<span class="red">missed</span>';
  };
  return `deadline ${cell(o.deadline_met)} · budget ${cell(o.budget_met, o.budget_over_pct)} · deliverable ${
    o.deliverable_confirmed === null || o.deliverable_confirmed === undefined

...
app.post('/webhooks/github', async (c) => {
  try {
    ...
    const result = await ingestGitHubCheckRun(
      body,
    ...
    const unauthorized = (error as Error).message.includes('signature');
...
runMigrations()
  .then(async () => {
    if (process.env.NODE_ENV === 'production') await verifyProofIngestorConnection();

...
async function seed() {
    await tx(async (client) => {
      ...
      RETURNING id`, [agentId, commitment.goal, hash]);
        const oathId = oath.rows[0].id;
        // lock the builder's identity — RCA outstanding, per the mechanism
        await client.query(`UPDATE agents SET locked = true, locked_oath_id = $2 WHERE id = $1`, [agentId, oathId]);
        await client.query(`INSERT INTO postmortems (oath_id, weight, agent_id, domain, failure_type,
...
//# sourceMappingURL=seed_0006.js.map
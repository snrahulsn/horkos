...
export async function verifyProofIngestorConnection() {
...
function requireIngestSecret(candidate) {
    const expected = process.env.PROOF_INGEST_SECRET;
    if (!expected)
        throw new Error('PROOF_INGEST_SECRET is not set');
...
async function recordAdapterResult(secret, raw, status) {
    ...
    const inserted = await db.query(`INSERT INTO proofs (
    ...
    const existing = await db.query(`SELECT * FROM proofs WHERE source = $1 AND external_id = $2`, [input.source, input.external_id]);
...
export function recordVerifiedAssertion(ingestSecret, input) {
...
export function recordRejectedAssertion(ingestSecret, input) {
...
export async function queryProofCoverage(raw) {
    ...
    const { rows } = await pool.query(`SELECT kind, source, status, count(*)::int AS count,
            max(observed_at) AS last_observed_at
     FROM proofs WHERE ${where.join(' AND ')}
     GROUP BY kind, source, status
     ORDER BY kind, source, status`, params);
    const verifiedKinds = new Set(rows.filter((row) => row.status === 'verified').map((row) => row.kind));
    return {
        proofs: rows,

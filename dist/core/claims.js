...
import { claimInputSchema, evidenceMatchesCriteria } from './guardrails.js';
 ...
 */
export async function logAttempt(agentId, milestoneId, model, modelVersion, outcome) {
    return tx(async (client) => {
        const { rows } = await client.query(`SELECT m.id, m.status, o.agent_id, o.ref, o.status AS oath_status
       FROM milestones m JOIN oaths o ON m.oath_id = o.id WHERE m.id = $1`, [milestoneId]);
        if (!rows.length)
            throw new GuardrailError(['unknown milestone']);
        const m = rows[0];
        if (m.agent_id !== agentId)
            throw new GuardrailError(['not your oath']);
        ...
        await client.query(`INSERT INTO attempts (milestone_id, model, model_version, outcome) VALUES ($1,$2,$3,$4)`, [milestoneId, model, modelVersion, outcome]);
        await logEvent(client, 'attempt.logged', { ref: m.ref, milestone: milestoneId, model, outcome });
        const count = await client.query(`SELECT count(*)::int AS n FROM attempts WHERE milestone_id = $1`, [milestoneId]);
 ...
 */
export async function fileClaim(agentId, milestoneId, raw) {
    const now = new Date();
    ...
    return tx(async (client) => {
        const { rows } = await client.query(`SELECT m.*, o.agent_id, o.ref, o.status AS oath_status, o.activated_at, o.id AS oath_id
       FROM milestones m JOIN oaths o ON m.oath_id = o.id WHERE m.id = $1 FOR UPDATE OF m`, [milestoneId]);
        if (!rows.length)
            throw new GuardrailError(['unknown milestone']);
        const m = rows[0];
        if (m.agent_id !== agentId)
        ...
        const gate = await client.query(`SELECT position FROM milestones
       WHERE oath_id = $1 AND position < $2
         AND status IN ('BROKEN','BROKEN_UNCONFIRMED') AND incident_filed = false
       ORDER BY position LIMIT 1`, [m.oath_id, m.position]);
        if (gate.rows.length) {
        ...
        const criteria = m.criteria_detail;
        const mismatch = evidenceMatchesCriteria(input.evidence, criteria);
        if (mismatch)
            throw new GuardrailError([`evidence rejected: ${mismatch}`]);
        if (criteria.type === 'github_check') {
            const proof = await client.query(`SELECT 1 FROM proofs WHERE milestone_id = $1 AND kind = 'outcome'
           AND source = 'github_check_run' AND status = 'verified' LIMIT 1`, [milestoneId]);
        ...
        const claim = await client.query(`INSERT INTO claims (milestone_id, evidence, actual_cost_usd, actual_duration_s, response_due)
       VALUES ($1,$2,$3,$4,$5) RETURNING id, response_due`, [
        ...
        await client.query(`UPDATE milestones SET status = 'CLAIMED', actual_cost_usd = $2, actual_duration_s = $3 WHERE id = $1`, [milestoneId, input.actual_cost_usd, input.actual_duration_s]);
        await client.query(`UPDATE oaths SET status = 'CLAIMED' WHERE id = $1 AND status = 'OPEN'`, [m.oath_id]);
...
export async function respondToClaim(counterpartyToken, claimId, response, disputeStatement) {
    const now = new Date();
    return tx(async (client) => {
        const { rows } = await client.query(`SELECT c.id AS claim_id, c.counterparty_response, m.id AS milestone_id, m.position,
       ...
       WHERE c.id = $1 FOR UPDATE OF c, m`, [claimId]);
        if (!rows.length)
            throw new GuardrailError(['unknown claim']);
        const r = rows[0];
        if (r.counterparty_token !== sha256Hex(counterpartyToken))
            throw new GuardrailError(['invalid counterparty token']);
        if (r.counterparty_response)
            throw new GuardrailError(['claim already resolved']);
        ...
        if (response === 'confirm') {
            ...
            await client.query(`UPDATE milestones SET status = $2, resolved_at = $3, deadline_met = $4, budget_met = $5 WHERE id = $1`, [r.milestone_id, verdict, now, deadlineMet, budgetMet]);
        ...
        await client.query(`INSERT INTO dispute_statements (claim_id, party, statement) VALUES ($1,'counterparty',$2)`, [claimId, disputeStatement.trim()]);
        await client.query(`UPDATE milestones SET status = 'DISPUTED', resolved_at = $2, deadline_met = $3, budget_met = $4 WHERE id = $1`, [r.milestone_id, now, deadlineMet, budgetMet]);
...
export async function respondToClaimAsOperator(authUserId, claimId, response, disputeStatement) {
    const { rows } = await pool.query(`SELECT op.id AS operator_id
     FROM claims c
     JOIN milestones m ON c.milestone_id = m.id
     JOIN oaths o ON m.oath_id = o.id
     JOIN agents a ON o.agent_id = a.id
     JOIN operators op ON a.operator_id = op.id
     WHERE c.id = $1 AND op.auth_user_id = $2`, [claimId, authUserId]);
    if (!rows.length)
        throw new GuardrailError(['unknown claim or not its owner']);
...
async function respondToClaimForOperator(operatorId, claimId, response, disputeStatement) {
    const now = new Date();
    return tx(async (client) => {
        const { rows } = await client.query(`SELECT c.id AS claim_id, c.counterparty_response, m.id AS milestone_id, m.position,
              m.deadline AS m_deadline, m.budget_slice_usd, m.actual_cost_usd, c.filed_at,
       ...
       JOIN oaths o ON m.oath_id = o.id
       WHERE c.id = $1 AND o.approved_by_operator_id = $2 FOR UPDATE OF c, m`, [claimId, operatorId]);
        if (!rows.length)
            throw new GuardrailError(['claim was not approved by this operator']);
        const r = rows[0];
        if (r.counterparty_response)
            throw new GuardrailError(['claim already resolved']);
        const deadlineMet = new Date(r.filed_at) <= new Date(r.m_deadline);
        const budgetMet = Number(r.actual_cost_usd) <= Number(r.budget_slice_usd);
        if (response === 'confirm') {
            await client.query(`UPDATE claims SET counterparty_response = 'confirm', responded_by_operator_id = $2 WHERE id = $1`, [claimId, operatorId]);
            const verdict = deadlineMet && budgetMet ? 'KEPT' : 'BROKEN';
            await client.query(`UPDATE milestones SET status = $2, resolved_at = $3, deadline_met = $4, budget_met = $5 WHERE id = $1`, [r.milestone_id, verdict, now, deadlineMet, budgetMet]);
            await logEvent(client, 'milestone.resolved', {
        ...
        if (!disputeStatement || disputeStatement.trim().length < 10)
            throw new GuardrailError(['dispute requires a statement (min 10 chars)']);
        await client.query(`UPDATE claims SET counterparty_response = 'dispute', responded_by_operator_id = $2 WHERE id = $1`, [claimId, operatorId]);
        await client.query(`INSERT INTO dispute_statements (claim_id, party, statement) VALUES ($1,'counterparty',$2)`, [claimId, disputeStatement.trim()]);
        await client.query(`UPDATE milestones SET status = 'DISPUTED', resolved_at = $2, deadline_met = $3, budget_met = $4 WHERE id = $1`, [r.milestone_id, now, deadlineMet, budgetMet]);
 ...
 */
export async function maybeResolveParent(client, oathId, now) {
    const ms = await client.query(`SELECT status, position, actual_cost_usd, resolved_at FROM milestones WHERE oath_id = $1 ORDER BY position`, [oathId]);
    const all = ms.rows;
    const terminal = all.every((x) => !['OPEN', 'CLAIMED'].includes(x.status));
    if (!terminal)
        return;
    const oath = await client.query(`SELECT ref, status, deadline, budget_cap_usd, agent_id, activated_at FROM oaths WHERE id = $1 FOR UPDATE`, [oathId]);
    const o = oath.rows[0];
    if (!['OPEN', 'CLAIMED'].includes(o.status))
        return; // already terminal
    const totalCost = all.reduce((s, x) => s + Number(x.actual_cost_usd ?? 0), 0);
    ...
    const anyDisputed = all.some((x) => x.status === 'DISPUTED');
    const anyUnconfirmed = all.some((x) => x.status === 'BROKEN_UNCONFIRMED');
    const deliverableConfirmed = last.status === 'KEPT';
    let verdict;
    if (anyDisputed)
        verdict = 'DISPUTED';
    else if (deliverableConfirmed && deadlineMet && budgetMet)
        verdict = 'KEPT';
    else if (anyUnconfirmed && last.status === 'BROKEN_UNCONFIRMED')
        verdict = 'BROKEN_UNCONFIRMED';
    else
        verdict = 'BROKEN';
    const durationS = o.activated_at
        ? Math.floor((lastResolved.getTime() - new Date(o.activated_at).getTime()) / 1000)
        : null;
    await client.query(`UPDATE oaths SET status = $2, resolved_at = $3, deadline_met = $4, budget_met = $5,
       budget_over_pct = $6, deliverable_confirmed = $7, actual_cost_usd = $8, actual_duration_s = $9
     WHERE id = $1`, [oathId, verdict, now, deadlineMet, budgetMet, budgetOverPct.toFixed(2), deliverableConfirmed, totalCost, durationS]);
    const brokenEnRoute = all.filter((x) => ['BROKEN', 'BROKEN_UNCONFIRMED'].includes(x.status)).length;
    ...
    if (verdict === 'BROKEN' || verdict === 'BROKEN_UNCONFIRMED') {
        await client.query(`UPDATE agents SET locked = true, locked_oath_id = $2 WHERE id = $1`, [o.agent_id, oathId]);
        await logEvent(client, 'agent.locked', { ref: o.ref });
    }
}
/** Agent's signed counter-statement on a disputed claim. */
export async function fileDisputeStatement(agentId, claimId, statement, signature) {
    return tx(async (client) => {
        const { rows } = await client.query(`SELECT o.agent_id, o.ref FROM claims c
       ...
       WHERE c.id = $1`, [claimId]);
        ...
        if (rows[0].agent_id !== agentId)
            throw new GuardrailError(['not your oath']);
        await client.query(`INSERT INTO dispute_statements (claim_id, party, statement, signature) VALUES ($1,'agent',$2,$3)`, [claimId, statement.trim(), signature]);
        return { filed: true };
    });
}

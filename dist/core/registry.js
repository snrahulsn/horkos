...
export async function registerAgent(authUserId, agentName, displayName) {
    return tx(async (client) => {
        ...
        const agent = await client.query(`INSERT INTO agents (operator_id, pubkey, privkey_enc, api_token_hash, name) VALUES ($1,$2,$3,$4,$5) RETURNING id`, [operatorId, pubkey, 'not-issued', sha256Hex(apiToken), agentName]);
...
export async function lookupAgent(pubkey) {
    ...
    if (!rows.length)
        return null;
    const agent = rows[0];
    const oaths = await pool.query(`SELECT o.ref, CASE WHEN o.visibility = 'public' AND NOT o.counterparty_withdrawn THEN o.task_title ELSE NULL END AS task_title,
...
export async function queryRegistry(opts) {
    ...
    const { rows } = await pool.query(`SELECT o.ref, CASE WHEN o.visibility = 'public' AND NOT o.counterparty_withdrawn THEN o.task_title ELSE NULL END AS task_title,
...
export async function getOath(ref) {
    ...
    const amendments = await pool.query(`SELECT field,
     ...
     FROM amendments WHERE oath_id = $1 AND approved_at IS NOT NULL ORDER BY approved_at`, [o.id, isPublic]);
    const proofKinds = await pool.query(`SELECT DISTINCT kind FROM proofs WHERE oath_id = $1 AND status = 'verified'`, [o.id]);
    const verifiedKinds = new Set(proofKinds.rows.map((row) => row.kind));
    const costVerified = verifiedKinds.has('cost');
    const verifiedModels = await pool.query(`SELECT DISTINCT assertion->>'model' AS model FROM proofs

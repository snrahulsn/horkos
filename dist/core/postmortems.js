...
function rejectDump(field, text) {
...
export async function filePostmortem(agentId, oathId, raw) {
    ...
    for (const [f, t] of Object.entries({
        ...
        if (dump)
            throw new GuardrailError([dump]);
    }
    return tx(async (client) => {
        const { rows } = await client.query(`SELECT ref, status, agent_id, domain, resolved_at FROM oaths WHERE id = $1`, [oathId]);
        if (!rows.length)
        ...
        const existing = await client.query(`SELECT 1 FROM postmortems WHERE oath_id = $1 AND weight = 'rca'`, [oathId]);
        if (existing.rows.length)
            throw new GuardrailError(['RCA already filed for this oath']);
        const pm = await client.query(`INSERT INTO postmortems (oath_id, weight, agent_id, domain, failure_type,
       ...
       VALUES ($1,'rca',$2,$3,$4,$5,$6,$7,$8,$9,$10) RETURNING id`, [
        ...
        const agent = await client.query(`SELECT locked, locked_oath_id FROM agents WHERE id = $1 FOR UPDATE`, [agentId]);
        let unlocked = false;
        if (agent.rows[0].locked && agent.rows[0].locked_oath_id === oathId) {
            // only unlock if no OTHER broken oath is missing its RCA
            const outstanding = await client.query(`SELECT o.id FROM oaths o
...
export async function fileIncident(agentId, milestoneId, raw) {
    ...
    return tx(async (client) => {
        const { rows } = await client.query(`SELECT m.id, m.position, m.status, m.incident_filed, o.ref, o.agent_id, o.domain
       FROM milestones m JOIN oaths o ON m.oath_id = o.id WHERE m.id = $1 FOR UPDATE OF m`, [milestoneId]);
        ...
        if (m.incident_filed)
            throw new GuardrailError(['incident note already filed']);
        const pm = await client.query(`INSERT INTO postmortems (milestone_id, weight, agent_id, domain, failure_type,
         what_broke, root_cause, for_future_agents)
       VALUES ($1,'incident',$2,$3,$4,$5,$6,$7) RETURNING id`, [milestoneId, agentId, m.domain, inc.failure_type, inc.what_broke, inc.root_cause, inc.lesson]);
...
export async function searchPostmortems(opts) {
    ...
    params.push(limit);
    const { rows } = await pool.query(`SELECT id, weight, domain, failure_type, summary, what_broke, root_cause,

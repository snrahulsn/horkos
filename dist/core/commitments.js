...
export async function activateOathAsOperator(oathId, authUserId) {
    const now = new Date();
    return tx(async (client) => {
        const { rows } = await client.query(`SELECT o.id, o.ref, o.status, o.draft_expires_at, o.commitment_hash, op.id AS operator_id
       FROM oaths o
       JOIN agents a ON o.agent_id = a.id
       JOIN operators op ON a.operator_id = op.id
       WHERE o.id = $1 AND op.auth_user_id = $2
       FOR UPDATE OF o`, [oathId, authUserId]);
        if (!rows.length)
            throw new GuardrailError(['unknown oath or not its owner']);
        const oath = rows[0];
        if (oath.status !== 'DRAFT')
            throw new GuardrailError([`oath is ${oath.status}, not DRAFT`]);
        if (new Date(oath.draft_expires_at) < now)
            throw new GuardrailError(['draft expired (24h)']);
        await client.query(`UPDATE oaths SET status = 'OPEN', activated_at = $2, void_until = $3,
         approved_by_operator_id = $4, approved_commitment_hash = commitment_hash,
         counterparty_token = NULL
       WHERE id = $1`, [oath.id, now, new Date(now.getTime() + VOID_WINDOW_MS), oath.operator_id]);
...
export async function rejectDraftAsOperator(oathId, authUserId) {
    return tx(async (client) => {
        const { rows } = await client.query(`SELECT o.id, o.ref, o.status FROM oaths o
       JOIN agents a ON o.agent_id = a.id JOIN operators op ON a.operator_id = op.id
       WHERE o.id = $1 AND op.auth_user_id = $2 FOR UPDATE OF o`, [oathId, authUserId]);
        if (!rows.length)
            throw new GuardrailError(['unknown oath or not its owner']);
        if (rows[0].status !== 'DRAFT')
            throw new GuardrailError([`oath is ${rows[0].status}, not DRAFT`]);
        await client.query(`UPDATE oaths SET status = 'DRAFT_EXPIRED', resolved_at = now(), counterparty_token = NULL WHERE id = $1`, [oathId]);

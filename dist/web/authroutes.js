...
import { supabaseConfig, authConfigured, verifyAccessToken, makeSession, readSession, sessionCookie, clearCookie, readCookie, } from './auth.js';
...
auth.get('/login', (c) => {
    if (!authConfigured()) {
        return c.html(layout('Sign in', `<h1>Sign-in unavailable</h1><div class="block">Sign-in is temporarily unavailable.</div>`), 503);
    }
...
auth.get('/dashboard', async (c) => {
    ...
    const agents = await pool.query(`SELECT a.pubkey, a.name, a.locked, a.created_at,
     ...
     WHERE op.auth_user_id = $1 ORDER BY a.created_at`, [user.id]);
    const pendingDrafts = await pool.query(`SELECT o.id, o.ref, o.task_title, o.goal, o.domain, o.task_type, o.complexity, o.risk_level,
    ...
    const pendingClaims = await pool.query(`SELECT c.id, o.ref, m.position, m.title, m.criteria_detail, c.evidence,
...
${pendingDrafts.rows.length ? pendingDrafts.rows.map((o) => `<div class="block">
...
<ol>${o.milestones.map((m) => `<li><b>${esc(m.title)}</b> — ${esc(JSON.stringify(m.criteria))} · ${esc(new Date(m.deadline).toISOString())} · $${esc(m.budget)}</li>`).join('')}</ol>
...
${pendingClaims.rows.length ? pendingClaims.rows.map((r) => `<div class="block">
...
auth.post('/dashboard/oaths/:id/approve', async (c) => {
    if (!sameOrigin(c))
        return c.text('Request could not be verified.', 403);
    const user = requireUser(c);
    if (!user)
        return c.redirect('/login');
    try {
        ...
        if (form['decision'] === 'reject')
            await rejectDraftAsOperator(c.req.param('id'), user.id);
        else
            await activateOathAsOperator(c.req.param('id'), user.id);
...
auth.post('/dashboard/claims/:id/respond', async (c) => {
    ...
    const form = await c.req.parseBody();
    try {
        await respondToClaimAsOperator(user.id, c.req.param('id'), form['response'] === 'dispute' ? 'dispute' : 'confirm', typeof form['statement'] === 'string' ? form['statement'] : undefined);
        return c.redirect('/dashboard');
    }
    catch (e) {
        const msg = e instanceof GuardrailError ? e.errors.join('; ') : 'error';
...
auth.post('/oaths/:ref/comments', async (c) => {
    ...
    const recent = await pool.query(`SELECT count(*)::int AS n FROM task_comments
     WHERE author_auth_user_id = $1 AND created_at > now() - interval '1 hour'`, [user.id]);
    ...
    const inserted = await pool.query(`INSERT INTO task_comments (oath_id, author_auth_user_id, body)
     SELECT id, $2, $3 FROM oaths
     WHERE ref = $1 AND visibility = 'public'
       AND status NOT IN ('DRAFT','DRAFT_EXPIRED')
     RETURNING id`, [ref, user.id, body]);

...
function oathTable(rows) {
    if (!rows.length)
        return '<div class="block meta">No oaths yet. Ref 0001 awaits.</div>';
    return `<table>
...
web.get('/oaths/:ref', async (c) => {
    ...
    if (!o)
        return c.html(layout('Not found', '<h1>Unknown oath</h1>'), 404);
    const body = `
...
${o.amendments.length
        ? `<h2>Amendments (bilateral)</h2><table><tr><th>Field</th><th>From</th><th>To</th><th>Approved</th></tr>${o.amendments
            .map((a) => `<tr><td>${esc(a.field)}</td><td>${esc(JSON.stringify(a.old_value))}</td><td>${esc(JSON.stringify(a.new_value))}</td><td class="meta">${fmtDate(a.approved_at)}</td></tr>`)
            .join('')}</table>`
        : ''}`;
...
function statsTable(rows) {
    if (!rows.length)
        return '<div class="block meta">No data in range.</div>';
    return `<table>
...
${roots
        .map((r) => `<tr><td>${r.from_seq}–${r.to_seq}</td><td><code>${esc(r.root)}</code></td><td class="meta">${fmtDate(r.signed_at)}</td></tr>`)
...
web.get('/respond/:token', async (c) => {
    ...
    const { rows } = await pool.query(`SELECT c.id, m.position, m.title, o.ref, c.evidence, c.actual_cost_usd, c.filed_at
     FROM claims c JOIN milestones m ON c.milestone_id = m.id JOIN oaths o ON m.oath_id = o.id
     WHERE o.counterparty_token = encode(sha256(convert_to($1, 'UTF8')), 'hex') AND c.counterparty_response IS NULL`, [token]);
    const body = `
<h1>Pending claims</h1>
${rows
        .map((r) => `<div class="block">
...
web.post('/respond/:token/:claimId', async (c) => {
    try {
        ...
        const r = await respondToClaim(c.req.param('token'), c.req.param('claimId'), form['response'] === 'dispute' ? 'dispute' : 'confirm', typeof form['statement'] === 'string' ? form['statement'] : undefined);
        return c.html(layout('Recorded', `<h1>Milestone ${r.milestone_position}: ${verdictBadge(r.verdict)}</h1><div class="block">The verdict is permanent.</div>`));
    }
    catch (e) {
        const msg = e instanceof GuardrailError ? e.errors.join('; ') : 'error';
        return c.html(layout('Error', `<h1 class="red">Refused</h1><div class="block">${esc(msg)}</div>`), 400);
